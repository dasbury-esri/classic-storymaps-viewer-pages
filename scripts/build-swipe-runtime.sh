...
RUNTIME_PATH="${RUNTIME_PATH:-runtimes/swipe/upstream/Swipe}"
OUTPUT_PATH="${OUTPUT_PATH:-runtimes/swipe/build}"
...
if [[ "$build_ok" == "true" && -d "$RUNTIME_PATH/deploy" ]]; then
  ...
  if [[ "$build_ok" != "true" ]]; then
    echo "Swipe grunt build failed on this toolchain; using source fallback output." >&2
  fi
  ...
  copy_item "$RUNTIME_PATH/web.config" "$OUTPUT_PATH"
fi

echo "Swipe build output copied to $OUTPUT_PATH"
