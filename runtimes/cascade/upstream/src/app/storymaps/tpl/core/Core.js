...
// put this up top so other css wil load after it and take precedent,
// except get it out of print because it has color: black !important;
// and background: none !important; on EVERYTHING. :fire:
// AND we need to do both !app.isPrint and the other thing for both
// grunt dev and build to work. :unamused:
// (it's also added to the builder and viewer build manifest because
// it won't get picked up this way... i'm sure there's a better way to do this...)
if (!app.isPrint) {
  require([
    app.isPrint ? '' : 'lib-build/css!lib/calcite-bootstrap/css/calcite-bootstrap-open.min',
    app.isPrint ? '' : 'lib-build/less!storymaps/tpl/view/ui/bootstrap-override'
  ]);
}
...
define([
  ...
  function init(mainView, builder) {
    ...
    */
    ...
    if (app.isInBuilder && (has('ie') || has('trident') || has('edge'))) {
      ...
      $('#fatalError .error-msg').html(i18n.viewer.errors.builderSupportOther.replace('${CHROME}', '<a href="https://www.google.com/chrome/" target="_blank">Chrome</a>').replace('${SAFARI}', '<a href="https://www.apple.com/safari/" target="_blank">Safari</a>').replace('${FIREFOX}', '<a href="https://www.mozilla.org/" target="_blank">Firefox</a>').replace('${ESRI-SUPPORT}', '<a href="https://support.esri.com" target="_blank">Esri Support</a>').replace('${BROWSER-SUPPORT-VOTE}', '<a href="http://links.esri.com/storymaps/vote-for-cascade-builder-browser-support" target="_blank">vote here</a>'));
      $('#fatalError').show();
      return;
    }
    ...
    if (app.isInBuilder && has('ipad')) {
      $('#fatalError .error-title').html(i18n.viewer.errors.sorry);
      $('#fatalError .error-msg').html(i18n.viewer.errors.builderSupportIpad);
      $('#fatalError').show();
      return;
    }
  ...
  function initStep2(builder) {
    ...
    // Fix for multiple twitter bootstrap popup to be open simultaneously (but make sure modal exists first -- it doesn't in print)
    if ($ && $.fn && $.fn.modal) {
    ...
    app.portal.on('load', function(response) {
      ...
      app.isSelfHosted = !app.isPortal && !CommonHelper.isArcGISOnlineHosted();
